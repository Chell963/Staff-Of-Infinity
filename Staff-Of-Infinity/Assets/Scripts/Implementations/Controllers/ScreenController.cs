...
namespace Implementations.Controllers
{
    ...
    public struct ScreenValue
    {
        ...
        [field: SerializeField] public bool IsSubScreen { get; private set; }
        [field: SerializeField] public bool CanBeOpenedTwice { get; private set; }
    }
    public class ScreenController : Controller, IControllerContainer, IHolderContainer, IInitialization
    {
        ...
        {
            _screenHolder = (ScreenHolder)holdersToInject.ToList().Find(holder => holder is ScreenHolder);
        }
        ...
        private async Task<Screen> HandleScreen(ScreenValue screenValue)
        {
            ...
            switch (screenValue.IsSubScreen)
            {
                case true when IsScreenAlreadyOpened(screenValue.ScreenType):
                case false when IsScreenAlreadyOpened(screenValue.ScreenType):
                    Debug.LogWarningFormat("Trying to open {0} screen, but it's already opened", screenValue.ScreenType);
                    return _openedScreens.Find(screen => screen.ScreenType == screenValue.ScreenType);
                case false:
                    CloseAllScreens(selectedScene);
                    break;
            }
            ...
            screenToShow.InjectControllers(_controllers.ToArray());
            screenToShow.Show();
        ...
        private bool IsScreenAlreadyOpened(ScreenType screenType) =>
            _openedScreens.Count != 0 && _openedScreens[^1].ScreenType == screenType;
